...
public class CalendarManager {
	...
	public CalendarData getLastMonthCalendarData() {
		mCalendar.add(Calendar.MONTH, -1); //mCalendar 는 전역변수. month를 -1하고 함수호출
		return getCalendarData();
	}
	...
	public CalendarData getCalendarData() {
		...
		mCalendar.set(Calendar.DAY_OF_MONTH, 1); //day를 1로 초기화 하고
		int dayOfWeek = mCalendar.get(Calendar.DAY_OF_WEEK);
		int thisMonthLastDay = mCalendar.getActualMaximum(Calendar.DAY_OF_MONTH); //맥시멈 날짜는 넣음
		//==last/next/current month를 미리 구해놓음
		mCalendar.add(Calendar.MONTH, -1);
