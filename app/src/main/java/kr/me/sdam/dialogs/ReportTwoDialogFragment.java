...
public class ReportTwoDialogFragment extends DialogFragment {
...
//		getDialog().getWindow().getAttributes().y = -18;
		...
		View view = inflater.inflate(R.layout.dialog_report_two_layout, container, false);
				...
				if(type == 1){
					NetworkManager.getInstance().putSdamABan(MyApplication.getContext()
							...
							new OnResultListener<ABanInfo>() {
								...
								@Override
								public void onSuccess(Request request, ABanInfo result) {
									mOneAdapter.remove(mOneItem);
									Toast.makeText(getActivity(), "더 이상 보지 않기 처리가 완료 되었습니다.", Toast.LENGTH_SHORT).show();
									dismiss();
								}
