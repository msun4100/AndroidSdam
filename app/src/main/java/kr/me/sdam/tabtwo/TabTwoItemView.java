...
public class TabTwoItemView extends FrameLayout {
	public TabTwoItemView(Context context) {
		super(context);
		init();
	}

	public TabTwoItemView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	public OnClickListener viewListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.image_report:
				if (mListener != null) {
					mListener.onLikeClick(TabTwoItemView.this, mItem,
	...
	private void init() {
		LayoutInflater.from(getContext()).inflate(
				R.layout.item_common_list_layout, this);
	...
	public void setItemData(TabTwoResult item) {
		mItem = item;
		...
		iconLikeView
				.setImageResource(R.drawable.b_main_view_contents_icon_05_off);

		if (item.myGood == 1) {
			iconLikeView
					.setImageResource(R.drawable.b_main_view_contents_icon_05_on);
		} else {
			iconLikeView
					.setImageResource(R.drawable.b_main_view_contents_icon_05_off);
		}
		...
		bgResource = BgImage.getBgImage(Integer.parseInt(item.category),
				Integer.parseInt(item.image));
		background.setImageResource(bgResource);
	}
	public void setItemData(CommonResult item) {
		mItem = (TabTwoResult)item;
